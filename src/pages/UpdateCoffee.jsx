...
const UpdateCoffee = () => {
  ...
  const onSubmit = (coffee, event) => {
    ...
    fetch(`http://localhost:5000/coffee/${loadedCoffee._id}/update`, {
      method: "PUT",
      headers: {
      ...
      .then((data) => {
        if (data.modifiedCount > 0) {
          Swal.fire({
            ...
            imageHeight: 150,
            title: `${coffee.name} is Updated.`,
          });
        } else {
          Swal.fire({
            icon: "error",
            title: "Oops...",
            text: `Please Update any Filed`,
          });
        }
      });
  };
      ...
      <div className="bg-[#F4F3F0] px-[8.48%] lg:py-20 py-10 my-12  rounded-md space-y-7">
        ...
        <form onSubmit={handleSubmit(onSubmit)}>
          <div className="font-raleway grid lg:grid-cols-2 grid-cols-1 gap-6">
            {coffeeInputFields.map((coffeeInputField, idx) => (
              ...
              >
                ...
                <input
                  ...
                  type="text"
                  name="name"
                  defaultValue={loadedCoffee[coffeeInputField.lName]}
            ...
            <input
              className="cursor-pointer lg:col-span-2 bg-[#D2B48C] py-3 font-rancho text-2xl border-2 border-black rounded-md w-full"
              type="submit"
